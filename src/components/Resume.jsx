...
const Resume = () => {
  ...
  const certifications = [
    ...
    { name: "JavaScript Algorithms & Data Structures - freeCodeCamp", year: "2022", icon: "🟡" },
                ...
                <div key={index} className="relative group">
                  ...
                  <div className="ml-6 pl-6 border-l-2 border-[#2a3150] group-hover:border-[#5f7cff] transition-all">
                    <div className="bg-gradient-to-r from-[#0f1222] to-transparent p-5 rounded-2xl border border-[#262b44] group-hover:border-[#3b4eff]/50 transition-all">
                      ...
                      <p className="text-[#5f7cff] text-sm mb-2">{exp.company}</p>
                      <p className="text-[#7e8eff] text-xs mb-3">{exp.year}</p>
                      <ul className="space-y-1">
                        {exp.description.map((desc, i) => (
                          <li key={i} className="text-[#b9c3e6] text-sm flex items-start gap-2">
          ...
          <div className={`flex-1 transition-all duration-1000 delay-500 transform ${
            ...
            <div className="space-y-5 mb-10">
              {education.map((edu, index) => (
                <div key={index} className="bg-gradient-to-r from-[#0f1222] to-transparent p-5 rounded-2xl border border-[#262b44] hover:border-[#3b4eff]/50 transition-all">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-2xl">{edu.icon}</span>
                    <h4 className="text-lg font-bold text-white">{edu.degree}</h4>
                  </div>
                  <p className="text-[#5f7cff] text-sm mb-1">{edu.institution}</p>
            ...
            <div className="grid grid-cols-1 gap-3">
              {certifications.map((cert, index) => (
                <div key={index} className="flex items-center gap-3 bg-[#0f1222] p-3 rounded-xl border border-[#262b44] hover:border-[#5f7cff] transition-all">
                  <span className="text-2xl">{cert.icon}</span>
                  <div className="flex-1">
                    <p className="text-[#cdd9ff] font-semibold text-sm">{cert.name}</p>
